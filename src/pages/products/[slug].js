      ...
      <section className="container">
        ...
        <div className="row mb-5">
          ...
          <div className="col-md-4 mt-5 mt-sm-0 text-center">
            <a href="#tertarik" className={`button button-interest`}>
              Saya Tertarik
            </a>
            {data.productBarcode ? (
                  ...
                  {data.productBarcode.map((item, index) => {
                    return (
                      <div className="col-4 col-sm-12" key={index}>
                        <Image
                          src={item.productBarcodeImage}
                          width={100}
                          height={100}
                          alt={item.productBarcodeLabel}
                        />
                        <br />
                        <a
                          href={item.productBarcodeLink}
                          target="_blank"
                          rel="noreferrer"
                          className="mb-5"
                        >
        ...
        {!data.productSpecifications ? (
          ...
          <div className="row mt-5">
            ...
            <div className="col-sm-6">
              {data.productSpecifications.slice(0, 10).map((item) => (
                <div className="specification">
                  <span className="specification-label">
                    {item.productSpecificationLabel}
                  </span>
                  <span>{item.productSpecificationValue}</span>
                </div>
              ))}
            </div>
            <div className="col-sm-6">
              {data.productSpecifications.slice(10, 20).map((item) => (
                <div className="specification">
                  <span className="specification-label">
                    {item.productSpecificationLabel}
                  </span>
        ...
        {!data.productFloorplan ? (
          ...
          <div className={`row mt-5 floorplan`}>
            ...
            {data.productFloorplan.map((floorplan) => {
              return (
                <div className="col-sm-6">
                  <Image
                    src={floorplan}
